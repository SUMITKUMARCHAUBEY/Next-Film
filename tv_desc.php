...
if($_SERVER['REQUEST_METHOD'] == 'POST')
{
  ...
  header("location:/myimdb/tv_desc.php?added=true && cata=$fname");
...
$sql="SELECT * FROM `shows` WHERE name='$film';";
...
<div class="jumbotron bg-dark  text-light m-5 p-5" style="border-radius:25px;">
  ...
  '.$row['show_desc'].'</p><p class="mx-5 px-4"><b>IMDB Rating: '.$row['stars'].'/10</b></p></div></div>
  <hr class="my-4">
...
<form action="tv_desc.php?uid='.$uid.' && cata='.$film.'" method="post">
