...
if($size<=250000 && strtolower($exe)=="jpg"){
  ...
  $rat=$_POST["rating"];
  $gen=$_POST["genre"];
  $sql="SELECT * FROM `films` WHERE name='$title';";
  $result=mysqli_query($con,$sql);
  $num=mysqli_num_rows($result);
  if($num==0)
  {
    if($title.".jpg"==$iname){
 $sql="INSERT INTO `films` (`sl_no`, `name`, `yor`, `dir`, `actor`, `stars`, `genera`, `film_desc`) VALUES (NULL, '$title', '$yor', '$dir', '$act', '$rat', '$gen', '$desc');";
 $result=mysqli_query($con,$sql);
 $res=move_uploaded_file($temp_name,"partials/img/".$iname);
 if($result){
  header("location:/myimdb/_add_film.php?added=true");
 }
 else{
  echo "somthing went wrong!!";
      ...
      echo 'please match the title of film with the name of the image';
    }
}
else{
...
<body>';
   ...
   echo '
   <h1 class="text-center my-2">Add New Film</h1>
   
   <div class="container my-5" style="border: 2px solid green; border-radius: 25px;">
   <h2 class="my-5">Give Us The Film Details.</h2>
   
   
   
   <form action="/myimdb/_add_film.php" method="post" enctype="multipart/form-data">
    
   <div class="row mt-5 p-5">
     
    <div class="col">
    <label for="exampleFormControlTextarea1"><h3>Film Title</h3></label>
      <input type="text" class="form-control" placeholder="Film Title" name="title" id="title">
    </div>
    ...
    <label for="exampleFormControlTextarea1"><h3>Film Description</h3></label>
  ...
  <div class="col"><label for="exampleFormControlTextarea1"><h3>Genre</h3></label>
    ...
    <label class="mx-0" for="exampleFormControlFile1"><h3>Film Poster</h3><p>The image must be of the exact name as the film title and in .jpg format and also the size must be less then 250kb </p></label>
